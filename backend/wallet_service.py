# Wallet service for managing deposit addresses

# Default wallet addresses for different payment methods
DEFAULT_WALLETS = {
    'bitcoin': 'bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh',
    'ethereum': '0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb8',
    'usdt_trc20': 'TXYZopYRdj2D9XRtbG4uTdwZjX9c2V4h9q',
    'usdt_erc20': '0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb8',
    'bank_transfer': {
        'bank_name': 'Chase Bank',
        'account_name': 'Monacap Trading Pro LLC',
        'account_number': '1234567890',
        'routing_number': '021000021',
        'swift_code': 'CHASUS33'
    },
    'paypal': 'payments@monacaptradingpro.com'
}

def get_wallet_address(method: str):
    """Get wallet address for a payment method"""
    return DEFAULT_WALLETS.get(method.lower(), None)

def get_all_wallets():
    """Get all wallet addresses"""
    return DEFAULT_WALLETS
